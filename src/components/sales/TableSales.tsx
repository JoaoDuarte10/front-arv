...
import { KeyboardArrowDown, KeyboardArrowUp, Delete, CheckCircle, Cancel } from '@material-ui/icons';
...
function Row(props: { row: ReturnType<typeof createData>, salesService: SalesService }) {
    const { row, salesService } = props;
    const [open, setOpen] = useState<boolean>(false);
    const [alert, setAlert] = useState<JSX.Element | null>(null);

    const registerPayment = async (idsales: number) => {
        const { success, error, notFound } = await salesService.registerPayment(idsales);

        if (success) {
            setAlert(<AlertSuccess title="Pagamento registrado! Atualize a pesquisa" />);
        };
        if (notFound) {
    ...
    return (
        <React.Fragment>
            <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
                ...
                <TableCell align="center">
                    {row.info[0].payment_status === "PAID" ? <CheckCircle htmlColor='#5cb85c' /> : <Cancel htmlColor='#f0ad4e' />}
                </TableCell>
            </TableRow>
            <TableRow>
                            ...
                            >
                                {row.info[0].payment_status === "PAID" ? null : (
                                    <button className='btn btn-success'
                                        ...
                                        onClick={(e: React.SyntheticEvent) => registerPayment(row.idsales)}
                                    >
                                        Registrar Pagamento
                                    </button>
                                )}
                                <BasicDeleteModal
                                    btnName="Excluir"
                                    onDeleteClient={async (e: React.SyntheticEvent) => deleteSale(row.idsales)}
                                >
                                    <Typography
                                ...
                                <TableHead>
                                    <TableRow sx={{ fontFamily: 'Montserrat' }}>
                                        ...
                                        <TableCell align="right" variant="body">
                                            ...
                                            >
                                                {row.info[0].payment_status === "PAID" ? "Data do pagamento" : "Previsão de pagamento"}
                                            </strong>
                                        </TableCell>
                                        <TableCell align="right" variant="body">
                                            <strong
